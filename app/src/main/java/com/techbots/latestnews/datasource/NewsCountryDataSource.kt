...
class NewsCountryDataSource(var apiInterface: APIInterface,
                            coroutineContext: CoroutineContext,
                            var country:String) : PageKeyedDataSource<Int, NewsArticle>() {
    private val job = Job()
    ...
    fun loadInitData(pageCount:Int, callback: LoadInitialCallback<Int, NewsArticle>) {
        ...
        scope.launch {
            try {
                val response = apiInterface.getTopHeadLines(country, API_KEY, pageCount)
                when{
